...
class DefaultProcessRecordsTask(
                ...
                val batch =
                    try {
                        file.localFile.inputStream().use {
                            val records =
                                if (file.isEmpty) {
                                    emptyList<DestinationRecordMarshaled>().listIterator()
                                } else {
                                    it.toRecordIterator()
                                }
                            val batch =
    ...
    private fun InputStream.toRecordIterator(): Iterator<DestinationRecordMarshaled> {
        return lineSequence()
            .map {
                when (
                    val message = deserializer.deserialize(it, DestinationRecordMarshaled::class)
                ) {
            ...
            .map { it as DestinationRecordMarshaled }
            .iterator()
    }
}
